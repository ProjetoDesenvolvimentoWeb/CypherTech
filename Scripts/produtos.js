...
function carregaElementosNaPagina(json) {
  ...
  for(let produto of json) {
    ...
    td5.innerHTML = <button class="visualizar" onclick="redirectEditar(${produto.id})">Editar</button>;
    tr.appendChild(td5);

    let td6 = document.createElement('td');
    td6.innerHTML =  <button class="excluir" onclick="redirectExcluirProduto(${produto.id})">Excluir</button>;
    tr.appendChild(td6);
...
function redirectAtividades(id){
  window.location.href = "atividades1.php?id="+id;
...
function redirectExcluirTurma(id){
  ...
  if (resposta == true) {
    window.location.href = "php/excluirTurma.php?id="+id;
  }
